...
const handleCreateBucket = () => {
  ...
  bucketStore.createBuckets({
    bucket: form.name,
    enable_quota: form.quota.size !== 0,
    quota_bytes: size,
      ...
      <el-row>
        ...
        <el-col :span="10" class="right">
          ...
          <el-row>
            <el-col>
              <div class="tips">
                TinyOSS uses buckets to organize objects. A bucket is similar to a folder or directory in a filesystem,
                where each bucket can hold an arbitrary number of objects.<br><br>
                <b>Versioning</b>allows to keep multiple versions of the same object under the same key.<br><br>
                <b>Object Locking</b> prevents objects from being deleted. Required to support retention and legal hold.
                Can only be enabled at bucket creation.<br><br>
                <b>Quota</b> limits the amount of data in the bucket.<br><br>
                <b>Retention</b> imposes rules to prevent object deletion for a period of time. Versioning must be
                enabled in order to set bucket retention policies.<br><br>
              </div>
            </el-col>
          </el-row>
        </el-col>
